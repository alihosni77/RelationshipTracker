...
app.use(cors({
  ...
  origin(origin, callback) {
    if (!origin || (env.NODE_ENV !== 'production' && allowedOrigins.has('*')) || allowedOrigins.has(origin)) return callback(null, true);
...
const tokenFor = (userId: string) => jwt.sign(
  { sub: userId },
  env.JWT_SECRET,
  { expiresIn: '15m', issuer: 'relationship-tracker', audience: 'relationship-client' },
);
...
function authRateLimit(req: Request, res: Response, next: NextFunction) {
  ...
  entry.count += 1;
  if (entry.count > 20) return res.status(429).json({ error: 'too_many_attempts', retryAfterSeconds: Math.ceil((entry.resetAt - now) / 1000) });
  return next();
}
...
app.post('/v1/auth/register', authRateLimit, async (req, res, next) => {
  try {
