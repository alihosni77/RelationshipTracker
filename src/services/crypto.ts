...
async function deriveKey(passphrase: string, salt: Uint8Array, usage: 'encrypt'|'decrypt') {
  ...
  return crypto.subtle.deriveKey({ name:'PBKDF2', salt, iterations:120000, hash:'SHA-256' }, material, { name:'AES-GCM', length:256 }, false, [usage]);
}
...
export async function decryptWithPassphrase(ciphertext: string, keyEnvelope: string, passphrase: string) {
  ...
  try { const opened=await crypto.subtle.decrypt({name:'AES-GCM',iv:base64ToBytes(envelope.iv)},key,base64ToBytes(ciphertext)); return new TextDecoder().decode(opened); } catch { throw new Error('invalid_message_password'); }
}
