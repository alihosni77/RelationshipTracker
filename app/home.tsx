...
const modules = [
  ...
  ['📅', 'Events', 'Calendar and countdowns', '/events'],
  ['✦', 'Relationship pulse', 'Balanced feedback', '/ratings'],
  ['✨', 'Date ideas', 'Make a plan together', '/activities'],
] as const;
...
export default function HomeScreen() {
  ...
  return <ScrollView style={s.screen} contentContainerStyle={s.content}>
    ...
    <Text style={s.section}>EXPLORE</Text><View style={s.grid}>{modules.map(([icon, title, detail, route]) => <Pressable key={title} onPress={() => router.push(route)} style={s.card}><Text style={s.icon}>{icon}</Text><Text style={s.cardTitle}>{title}</Text><Text style={s.detail}>{detail}</Text></Pressable>)}</View>
    <Text style={s.section}>WELLBEING</Text><Pressable onPress={() => router.push('/wellbeing')} style={s.well}><Text style={s.wellIcon}>✦</Text><View style={s.flex}><Text style={s.cardTitle}>Cycle-aware support</Text><Text style={s.detail}>Private tracking and optional caring reminders.</Text></View><Text style={s.open}>Open</Text></Pressable><Text style={s.privacy}>Private by design · Every shared feature is opt-in · Pause sharing anytime</Text>
  </ScrollView>;
}
